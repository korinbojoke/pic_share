...
(function() {
  this.select_kind = function(kind) {
    ...
    for (i = 0, len = colors.length; i < len; i++) {
      col = colors[i];
      results.push($('[name="color"]').append($('<option>').val(col).text(col)));
    }
    return results;
  };

  this.select_color = function() {
    var col, i, item, items, len, results;
    $('[name="item"]').empty();
    col = $('[name="color"]').val();
    switch (col) {
      case "green":
    ...
    for (i = 0, len = items.length; i < len; i++) {
      item = items[i];
      results.push($('[name="item"]').append($('<option>').val(item).text(item)));
    }
    return results;
  };
